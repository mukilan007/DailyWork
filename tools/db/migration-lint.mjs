// Shared migration lint — usable by any app in this repo.
...
//
// Usage (from an app dir, e.g. daily-rhythm/ or todo-app/):
//   node ../tools/db/migration-lint.mjs
//
// Or with a custom migrations dir:
//   node ../tools/db/migration-lint.mjs path/to/migrations
//
...
try {
  ...
  if (e.code === "ENOENT") {
    ...
    console.error("Run from an app directory (e.g. cd daily-rhythm && npm run db:lint).");
    process.exit(1);
  }
  throw e;
}
...
console.log(`Lint OK — scanned ${files.length} migration file(s) in ${DIR}.`);
